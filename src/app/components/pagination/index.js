...
class Pagination extends Component {
  ...
  handleGo = () => {
    ...
    let inputPage =
      this.refs.msPaginationGoInput && this.refs.msPaginationGoInput.value
  ...
  // 计算页码，返回页码元素
  calculatePages() {
    let { middlePage } = this.props
    const { currentPage, pageCount } = this.state
    let fixPage // 根据middlePage修正页码
    let viewPageStart // 中间页码开始
    let viewPageEnd // 中间页码结束
    let ret = []

    if (pageCount <= 7) {
      middlePage = Math.ceil(pageCount / 2)
    }
    ...
    if (viewPageStart > 2) {
      ...
      if (pageCount > 7) {
        ret.push(
          <li key={'pagination-firt-dot'} className='ms-pagination-dot'>
            ...
          </li>,
        )
      }
    ...
    if (viewPageEnd !== pageCount) {
      if (pageCount > 7) {
        ret.push(
          <li key={'pagination-last'} className='ms-pagination-dot'>
            ...
          </li>,
        )
      }
          ...
          {showGo ? (
            <div className='ms-pagination-go-wrap'>
              <div className='ms-pagination-go'>
                <span>跳转到</span>
                <input className='form-input' ref='msPaginationGoInput' />
                <span>页</span>
              </div>
              <button
                className='btn btn-primary ms-pagination-btn'
